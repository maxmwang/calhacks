  ...
  return (
    <div>
      <div id="component">
        <div id="form">
          <h1>Join Party!</h1>
          <input placeholder="Enter Party Code" value={code} onChange={(e) => setCode(e.target.value)} />
          <input placeholder="First Name" value={firstName} onChange={(e) => setFirstName(e.target.value)} />
          <input placeholder="Last Name" value={lastName} onChange={(e) => setLastName(e.target.value)} />
        </div>
        <button type="submit" id="next" onClick={() => dispatch(partyJoin(code, self.id))}>Next</button>
      </div>
    </div>
  );
}
